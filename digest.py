...
OVERDUE_JOB_DAYS = 14  # flag jobs with no status change for this many days
...
SQL_APPS_OVERDUE = """
    ...
    FROM applications
    WHERE follow_up_needed = TRUE
      AND status IN ('Applied', 'Under review', 'HR screen',
                     'Tech interview 1', 'Tech interview 2', 'Final round')
      AND (CURRENT_DATE - last_status_change) >= :threshold
...
SQL_WEEKLY_SUMMARY = """
    SELECT
      ...
      (SELECT COUNT(*) FROM applications
         WHERE status NOT IN ('Rejected', 'Withdrew', 'Ghosted', 'Offer')) AS active_pipeline
"""
...
def render_table(rows, columns):
    ...
    body = ""
    for r in rows:
        cells = "".join(f"<td style='padding:6px 10px;border-bottom:1px solid #eee'>{r.get(c, '') if r.get(c) is not None else ''}</td>"
                        for c in columns)
        body += f"<tr>{cells}</tr>"
    return f"<table style='border-collapse:collapse;font-family:sans-serif;font-size:14px'><thead><tr>{head}</tr></thead><tbody>{body}</tbody></table>"

def build_html():
    due       = fetch_rows(SQL_OUTREACH_DUE)
    overdue_o = fetch_rows(SQL_OUTREACH_OVERDUE)
    overdue_a = fetch_rows(SQL_APPS_OVERDUE, {"threshold": OVERDUE_JOB_DAYS})
    ...
    <html><body style="font-family:sans-serif;max-width:800px;margin:auto;color:#222">
      ...
      <h3 style="color:#d93025">🚨 Job applications stale ≥{OVERDUE_JOB_DAYS} days ({len(overdue_a)})</h3>
      {render_table(overdue_a, ['id','job_title','company','status','last_status_change','days_stale'])}
      ...
      <p style="color:#888;font-size:12px;margin-top:30px">
        To silence a row: open the Streamlit app → toggle "Follow-up needed" off.<br>
        To mark a follow-up as sent: same app → "Mark follow-up sent today".
      </p>
    </body></html>
    """
    return html
